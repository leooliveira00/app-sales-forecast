...
import type { UnidadeVenda } from '../../types';
...
export const EditarUnidadeModal: React.FC<EditarUnidadeModalProps> = ({
  unidade, token, onClose, onUpdated, showToast,
}) => {
  const [descricao, setDescricao] = useState(unidade.descricao);
  const [tipo, setTipo]           = useState<'NACIONAL' | 'EXPORT'>((unidade.tipo as 'NACIONAL' | 'EXPORT') ?? 'NACIONAL');
  const [ativo, setAtivo]         = useState(unidade.ativo);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`/api/unidades/${unidade.codigo}`, {
        method: 'PUT',
      ...
      if (res.ok) {
        ...
        showToast(err.error || 'Erro ao atualizar unidade', 'error');
      }
    } catch {
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-md relative">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h3 className="font-bold text-slate-900 text-lg">Editar Unidade</h3>
            <p className="text-xs text-slate-400 font-mono">{unidade.codigo}</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex items-center gap-3 py-1">
            ...
            />
            <label htmlFor="ativo-check" className="text-sm text-slate-600 cursor-pointer select-none">
              Unidade ativa
            </label>
          </div>
