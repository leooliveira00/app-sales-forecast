...
interface FamilySalesDetailProps {
  label: string;
  avgTrim: number | null;
  avgSem: number | null;
  avg12m: number | null;
  salesHistory: Array<{ month: string; qty: number }>;
  orcHistory: Array<{ month: string; volumeORC: number }>;
  fcts?: number | null;
...
export const FamilySalesDetail: React.FC<FamilySalesDetailProps> = ({
  label, avgTrim, avgSem, avg12m, salesHistory, orcHistory, fcts, fctsHistory,
}) => {
  const { t } = useTranslation('forecast');
  ...
  const orcByMonth  = new Map(orcHistory.map(o => [monthLabel(o.month), o.volumeORC]));
  const fctsByMonth = new Map((fctsHistory ?? []).map(f => [monthLabel(f.month), f.fcts]));

  const data = salesHistory.map(h => {
    const lbl = monthLabel(h.month);
    return {
      month: lbl,
      qty:   h.qty,
      orc:   orcByMonth.get(lbl) ?? null,
      fcts:  fctsHistory ? (fctsByMonth.get(lbl) ?? null) : (fcts ?? null),
    };
  });

  const hasData      = data.length > 0;
  const hasOrc       = orcHistory.length > 0;
  ...
  return (
    <div className="px-6 py-4 bg-sky-50/40 border-b border-sky-100">
      <p className="text-[10px] font-bold text-sky-600 uppercase tracking-wider mb-3">
        {t('salesChart.analyticsFamily', { label })}
      </p>
        ...
        {hasData ? (
          <div className="flex-1 min-w-[240px]">
            <p className="text-[10px] text-slate-400 uppercase tracking-wider mb-2">
              {t('salesChart.chart.aggregatedTitle', { count: data.length })}
              {hasOrc && (
